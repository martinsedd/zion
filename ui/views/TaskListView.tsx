...
export interface TaskListViewRef {
  refresh: () => void;
}

export const TaskListView = forwardRef<TaskListViewRef, TaskListViewProps>(
  ({ onAddTask, onExit }, ref) => {
    const [tasks, setTasks] = useState<Task[]>([]);
    const [selectedIndex, setSelectedIndex] = useState(0);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const getTasksUseCase = container.get<GetTasksUseCase>(
      TYPES.GetTasksUseCase,
    );
